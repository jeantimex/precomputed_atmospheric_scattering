...
5. **[ ] Sphere Masking**: Add the analytic sphere intersection and render it with a flat albedo so you can verify silhouettes and depth ordering.
   ...
   - **[ ] 5.2 Add Ground Surface Intersection**: Port ground intersection with earth surface, handle depth ordering (ground behind sphere). Visual target: a matte-colored planet horizon clipped by an analytic sphere so the ground fills the lower half of the frame except where the debug sphere occludes it in front.
