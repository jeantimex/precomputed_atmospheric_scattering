...
1. Replace WebGL texture helpers with WebGPU equivalents:
   - Create `GPUTexture` objects for transmittance & irradiance (2D) and scattering (3D).
   - Set usage flags `TEXTURE_BINDING | COPY_DST`.
   - Upload `.dat` payloads with `device.queue.writeTexture`.
...
1. **Context + Clear Pass**: Run `npm run dev`, navigate to the WebGPU demo, confirm the canvas clears to blue (or chosen debug color).
2. **Fullscreen Quad**: Render a clip-space quad with a simple gradient shader (e.g., `color = vec4(position.xy * 0.5 + 0.5, 0, 1)`); verify resizing still works.
