...
/**
 * PointControlsManager class for handling pointer events
 ...
 */
export class PointControlsManager {
  /**
  ...
  onPointerMove(event) {
    ...
    if (this.drag === 'sun') {
      // Update sun position
      this.demo.sunZenithAngleRadians -= (this.previousPointerY - pointerY) / kScale;
      this.demo.sunZenithAngleRadians = Math.max(0, Math.min(Math.PI, this.demo.sunZenithAngleRadians));
      this.demo.sunAzimuthAngleRadians += (this.previousPointerX - pointerX) / kScale;
      
      // Update sun direction in the shader
      const sunDirection = new THREE.Vector3(
        Math.sin(this.demo.sunZenithAngleRadians) * Math.cos(this.demo.sunAzimuthAngleRadians),
        Math.sin(this.demo.sunZenithAngleRadians) * Math.sin(this.demo.sunAzimuthAngleRadians),
      ...
      this.demo.viewZenithAngleRadians += (this.previousPointerY - pointerY) / kScale;
      this.demo.viewZenithAngleRadians = Math.max(0, Math.min(Math.PI / 2, this.demo.viewZenithAngleRadians));
      this.demo.viewAzimuthAngleRadians += (this.previousPointerX - pointerX) / kScale;
  ...
  isPointerOverSun(event) {
    ...
    raycaster.setFromCamera(new THREE.Vector2(normalizedX, normalizedY), this.demo.camera);
    
    // Get the sun direction in world space
    const sunDirection = this.demo.material.uniforms.sun_direction.value.clone();
    ...
    const sunAngularRadius = Math.atan(this.demo.material.uniforms.sun_size.value.x);
