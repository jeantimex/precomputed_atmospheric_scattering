...
1. Re-read `webgpu/README.md`, `webgpu/MIGRATION.md`, and `webgpu/preset_report.md` to capture required behaviours, bindings, and parity expectations.
2. Confirm the following with the user (document answers inline):
   - OK to rely on Three.js `WebGPURenderer` (experimental) pinned to a specific release.
   - WGSL ingestion path preference (`RawShaderMaterial` vs node materials).
   - UX parity requirements (reuse existing controls/help overlay vs. adopt OrbitControls).
   - Initial scope limited to fullscreen quad + post-pass (i.e., no arbitrary geometry yet).

> **Verification:** Add a short “Scope Decisions” subsection in this file capturing the confirmed answers so future contributors know the constraints.
...
1. **Scaffold entry point**
   ...
   - Verification: `npm run dev -- --open /three-webgpu/` loads the page with canvas + overlays, but no rendering yet.
2. **Install deps**
   ...
   - Verification: `npm install` updates lockfile without breaking existing scripts; `npm run lint` (if available) still passes.
3. **Hello WebGPU via Three.js**
   ...
   - Verification: Running `/three-webgpu/` shows a colored quad, and browser devtools confirm WebGPU is in use (no fallback to WebGL).
...
1. **Port LUT loader**
   ...
   - Verification: Temporary UI logs texture dimensions + sample values; console asserts formats are `RGBA32F`.
2. **Shared uniform buffer**
   ...
   - Verification: Unit-like test (or console log) showing the buffer matches `/webgpu` output for a known preset (compare arrays).
3. **Fullscreen quad material stub**
   ...
   - Verification: Changing a uniform (e.g., exposure) affects the color, proving bindings work.
...
1. **WGSL ingestion**
   ...
   - Verification: Stub shader from previous step compiles without validation warnings.
2. **Port vertex shader**
   ...
   - Verification: Output interpolated view rays as RGB and compare with `/webgpu` debug output (e.g., log values for center pixel).
3. **Port fragment shader**
   ...
   - Verification: Preset “1” screenshot from `/webgpu` vs `/three-webgpu` shows matching colors (within tolerance). Log `GetCombinedScattering` sample results to ensure no NaNs.
...
1. **Shared state module**
   ...
   - Verification: Unit-style comparison of preset data structures between folders.
2. **Controls wiring**
   ...
   - Verification: Console log state values while performing each interaction; confirm they match logs from `/webgpu` for the same gestures.
3. **UI overlay reuse**
   - Share the help/status HTML/CSS so `/three-webgpu/` looks identical.
   - Verification: Toggling `h`, pressing `=`/`-`, etc., manipulates the same DOM elements with no layout regressions.
...
1. **Two-pass pipeline**
   ...
   - Verification: Toggle the post pass to observe difference; with the pass disabled you see raw geometry, with it enabled you see atmospheric haze plus sky at depth=1.
2. **Replace analytic sphere**
   ...
   - Verification: Side-by-side screenshots before/after show the sphere looks identical (position, shading) but now responds to standard Three.js material tweaks (e.g., change albedo).
3. **Geometry regression cases**
   ...
   - Verification: Render both the sphere and the torus; confirm they respect the same atmospheric attenuation, cast/receive the post haze correctly, and that presets still behave.
4. **Normal buffer (optional)**
   ...
   - Verification: Visualize the normal buffer on screen to ensure encoding is correct before using it.

---

## 6. Testing & Parity Checklist
1. **Preset parity sweep**
   ...
   - Verification: Add a new `/three-webgpu/preset_report.md` documenting any deviations.
2. **Performance sanity check**
   - Use browser performance profiler to record frame times with presets 1 and 6 (low sun vs. high exposure).
   - Verification: Note FPS numbers in the plan or README to ensure no regressions >10% compared to `/webgpu`.
3. **Fallback + UX**
   - Temporarily disable `navigator.gpu` via devtools to confirm the status overlay shows the same message as `/webgpu`.
   - Verification: Document the message text in README so QA knows what to expect.
...
1. Confirm (and log) whether `/webgpu` stays untouched long-term, with `/three-webgpu` as an alternate entry, or if the latter will eventually replace the former.
2. Approve the initial milestone order (Sections 1 → 4) before writing code; defer Section 5 until sky parity is verified.
3. Decide whether to integrate Three.js meshes immediately after parity or ship parity first.

**Verification:** Add a “Decision Log” appendix capturing the agreed answers + dates so future contributors know the plan history.
